...
export function InferenceResultViewer({ rows = [], initialIndex = 0, onClose, predictionItems, predictionBoxStyle, predictionColor }) {
  ...
  return <div className={`viewer-overlay evaluation-sample-dialog inference-result-dialog viewer-${theme}`}>
    ...
    <div className="viewer-stage shared-image-viewport" onWheel={(event) => { if (!event.ctrlKey) return; event.preventDefault(); zoom(event.deltaY < 0 ? .2 : -.2); }} onMouseDown={(event) => setDrag({ x: event.clientX, y: event.clientY, pan })} onMouseMove={(event) => drag && setPan({ x: drag.pan.x + event.clientX - drag.x, y: drag.pan.y + event.clientY - drag.y })} onMouseUp={() => setDrag(null)} onMouseLeave={() => setDrag(null)}><div className="shared-image-canvas" style={{ "--image-ratio": Number(row.image_width || 16) / Number(row.image_height || 9), aspectRatio: `${Number(row.image_width || 16)} / ${Number(row.image_height || 9)}`, transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})` }}><AuthenticatedImage src={imageSrc} alt={row.display_name || "推理结果"} draggable="false" onSourceReady={() => setImageLoaded(true)} /><div className={`viewer-box-layer ${imageLoaded ? "ready" : "loading"}`}>{imageLoaded && predictionItems(row.predictions_json).map((prediction, predictionIndex) => { const style = predictionBoxStyle(prediction, row); if (!style) return null; const color = predictionColor(prediction.label); return <i className="prediction-box" key={prediction.id || predictionIndex} style={{ ...style, borderColor: color, "--box-color": color }}>{prediction.score != null && <small>{(Number(prediction.score) * 100).toFixed(0)}%</small>}</i>; })}</div></div></div>

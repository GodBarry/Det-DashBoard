    ...
    const result = await query(
      `WITH RECURSIVE scoped_projects AS (
       ...
       annotation_counts AS (
         ...
         FROM subtree
         JOIN project_images pi ON pi.project_id = subtree.project_id AND pi.deleted_at IS NULL
         JOIN image_annotations a ON a.project_image_id = pi.id
         GROUP BY subtree.root_id
       ),
       import_times AS (
       ...
       SELECT p.*,
        ...
        COALESCE((SELECT jsonb_agg(DISTINCT a.label) FILTER (WHERE a.label IS NOT NULL AND a.label<>'' AND lower(trim(a.label))<>'mosaic') FROM subtree s JOIN project_images pi ON pi.project_id=s.project_id AND pi.deleted_at IS NULL JOIN image_annotations a ON a.project_image_id=pi.id WHERE s.root_id=p.id), '[]'::jsonb) AS labels,
        it.last_import_at
       FROM projects p
    ...
    const rows = await query(
      `WITH RECURSIVE subtree AS (
       ...
       SELECT
        ...
        (SELECT count(*)::int FROM image_annotations a JOIN project_images pi ON pi.id=a.project_image_id JOIN subtree s ON s.id=pi.project_id WHERE s.id=$1 AND pi.deleted_at IS NULL) AS direct_annotation_count,
        ...
        (SELECT count(DISTINCT a.project_image_id)::int FROM image_annotations a JOIN project_images pi ON pi.id=a.project_image_id JOIN subtree s ON s.id=pi.project_id WHERE pi.deleted_at IS NULL) AS labeled_image_count,
        (SELECT count(*)::int FROM image_annotations a JOIN project_images pi ON pi.id=a.project_image_id JOIN subtree s ON s.id=pi.project_id WHERE pi.deleted_at IS NULL) AS annotation_count,
        (SELECT COALESCE(json_agg(json_build_object('label', label, 'count', count) ORDER BY count DESC, label), '[]'::json)
         FROM (
           ...
           JOIN subtree s ON s.id=pi.project_id
        ...
        (SELECT json_agg(DISTINCT label) FROM image_annotations a JOIN project_images pi ON pi.id=a.project_image_id JOIN subtree s ON s.id=pi.project_id WHERE pi.deleted_at IS NULL AND lower(trim(a.label)) NOT IN ('no', 'none', 'background', 'bg', 'negative', 'mosaic')) AS labels`,
      [projectId],
    );
    return rows.rows[0];
  }

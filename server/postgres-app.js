...
async function resolveTrainingAlgorithmSource(params = {}) {
  return algorithmRuntimeSource.resolveTrainingAlgorithmSource(params);
}

function assetPathSegmentForCache(value) {
  return algorithmRuntimeSource.assetPathSegmentForCache(value);
}

function findFileUnder(root, predicate) {
  return algorithmRuntimeSource.findFileUnder(root, predicate);
}

function ensureAlgorithmSourceArchiveExtracted(cacheRoot) {
  return algorithmRuntimeSource.ensureAlgorithmSourceArchiveExtracted(cacheRoot);
}

async function resolveDinoConfigPath({ env, cacheRoot, algorithm, params, weightPath, outputRoot }) {
  return algorithmRuntimeSource.resolveDinoConfigPath({ env, cacheRoot, algorithm, params, weightPath, outputRoot });
}

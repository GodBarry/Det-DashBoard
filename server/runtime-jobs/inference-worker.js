...
function createInferenceWorker({
  ...
  async function runDinoInferenceJob(job) {
    ...
    const runner = [
      ...
      "elif str(cfg['device']).startswith('cpu'):",
      "    active_batch = 1",
      "else:",
      ...
      "        is_oom = isinstance(error, torch.cuda.OutOfMemoryError) or 'out of memory' in str(error).lower() or 'alloc' in str(error).lower()",
      "        if not is_oom or active_batch <= 1: raise",
      "        active_batch = max(1, active_batch // 2)",
      "        if str(cfg['device']).startswith('cuda'): torch.cuda.empty_cache()",
      "        print('[DINO] accelerator OOM; reducing batch to %d and retrying' % active_batch, flush=True)",
      "        continue",
